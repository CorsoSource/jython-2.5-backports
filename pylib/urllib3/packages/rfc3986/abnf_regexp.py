...
URL_PARSING_RE = (
    r'(?:(?P<scheme>%(scheme)s):)?(?://(?P<authority>%(authority)s))?'
    r'(?P<path>%(path)s)(?:\?(?P<query>%(query)s))?'
...
LS32_RE = '(%(hex)s:%(hex)s|%(ipv4)s)' % dict(hex=HEXDIG_RE, ipv4=IPv4_RE)
...
IPv6_RE = '(%s)' % ('|'.join('(%s)' % v for v in variations),) 
...
IREGULAR_NAME_RE = IREG_NAME = u'(?:%s|[%s])*' % (
    u'%[0-9A-Fa-f]{2}', SUB_DELIMITERS_RE + IUNRESERVED_RE
)

IHOST_RE = IHOST_PATTERN = u'(%s|%s|%s)' % (
    IREG_NAME,
    IPv4_RE,
    IP_LITERAL_RE,
)
