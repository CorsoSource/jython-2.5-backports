...
class VerifiedHTTPSConnection(HTTPSConnection):
    ...
    def connect(self):
        ...
        if is_time_off:
            warnings.warn(
                'System time is way off (before %s). This will probably lead to SSL verification errors' % RECENT_DATE,
                SystemTimeWarning
            )
        ...
        if self.assert_fingerprint:
            ...
            if not cert.get('subjectAltName', ()):
                warnings.warn((
                    'Certificate for %s has no `subjectAltName`, falling back to check for a ' 
