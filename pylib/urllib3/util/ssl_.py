...
def assert_fingerprint(cert, fingerprint):
    ...
    if not hashfunc:
        raise SSLError(
            'Fingerprint of invalid length: %s' % (fingerprint,))
    ...
    if not _const_compare_digest(cert_digest, fingerprint_bytes):
        raise SSLError('Fingerprints did not match. Expected "%s", got "%s".' % (fingerprint, hexlify(cert_digest)))
...
def ssl_wrap_socket(sock, keyfile=None, certfile=None, cert_reqs=None,
    ...
    if ca_certs or ca_cert_dir:
        ...
        except IOError, e:  # Platform-specific: Python 2.7
            raise SSLError(e)
        # Py33 raises FileNotFoundError which subclasses OSError
        # These are not equivalent unless we check the errno attribute
        except OSError, e:  # Platform-specific: Python 3.3 and beyond
