...
PATH_CHARS = abnf_regexp.UNRESERVED_CHARS_SET | abnf_regexp.SUB_DELIMITERS_SET | set((':', '@', '/'))
QUERY_CHARS = FRAGMENT_CHARS = PATH_CHARS | set(('?',))
...
def _encode_invalid_chars(component, allowed_chars, encoding='utf-8'):
    ...
    is_percent_encoded = percent_encodings == uri_bytes.count('%')
    ...
    for i in range(0, len(uri_bytes)):
        ...
        byte_ord = ord(byte)
        if ((is_percent_encoded and byte == '%')
                or (byte_ord < 128 and byte.decode() in allowed_chars)):
            encoded_component.extend(byte)
            continue
        encoded_component.extend(('%%%02x' % (byte_ord)).encode().upper())
