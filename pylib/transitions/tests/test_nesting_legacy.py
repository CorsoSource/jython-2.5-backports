...
from transitions.tests.test_nesting import TestNestedTransitions, TestSeparatorsBase, Stuff, default_separator, test_states
from transitions.tests.test_reuse import TestReuse as TestReuse, TestReuseSeparatorBase
from transitions.tests.test_reuse import test_states as reuse_states
from transitions.tests.test_enum import TestNestedStateEnums
...
class TestNestedLegacy(TestNestedTransitions):
    ...
    def test_add_custom_state(self):
        ...
        s.machine.add_transition('run', 'E', 'C%s3%sa' % (self.state_cls.separator,self.state_cls.separator,))
        s.go()
    ...
    def test_init_machine_with_nested_states(self):
        ...
        self.assertEqual(b_1.name, 'B%s1' % (State.separator,))
...
class TestReuseLegacy(TestReuse):
    ...
    def test_reuse_self_reference(self):
        ...
        class Top(self.machine_cls):
            ...
            def __init__(self):
                ...
                transitions = [dict(trigger='print_top', source='*', dest='=', after=self.print_msg),
                               dict(trigger='to_nested', source='*', dest='B%s1' % (separator,))]
        ...
        self.assertIsNot(top_machine.nested.get_state('2').on_enter,
                         top_machine.get_state('B%s2' % (separator,)).on_enter)
