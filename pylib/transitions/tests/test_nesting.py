...
from transitions.tests.test_core import TestTransitions, TestCase
from transitions.tests.utils import Stuff, DummyModel
...
class TestNestedTransitions(TestTransitions):
    ...
    def test_init_machine_with_nested_states(self):
        ...
        m.to("B%s1" % (State.separator,))
        self.assertEqual(m.state, "B%s1" % (State.separator,))
    ...
    def test_nested_definitions(self):
        ...
        m = self.stuff.machine_cls(initial='A', states=['A', state],
                                   transitions=[['go', 'A', 'B'], ['go', 'B%s2' % (separator,),
                                                                   'B%s1' % (separator,)]])
        self.assertTrue(m.is_A())
        m.go()
        self.assertEqual(m.state, 'B%s2' % (separator,))
        m.go()
        self.assertEqual(m.state, 'B%s1' % (separator,))
        m.jo()
    ...
    def test_add_custom_state(self):
        ...
        s.machine.add_transition('run', 'E', 'C%s3%sa' % (State.separator,State.separator,))
        s.go()
        self.assertEqual('E%s1' % (State.separator,), s.state)
        s.walk()
        self.assertEqual('E%s3' % (State.separator,), s.state)
        s.run()
        self.assertEqual('C%s3%sa' % (State.separator,State.separator,), s.state)

    def test_add_nested_state(self):
        m = self.machine_cls(states=['A'], initial='A')
        m.add_state('B%s1%sa' % (self.state_cls.separator,self.state_cls.separator,))
    ...
    def test_enter_exit_nested_state(self):
        ...
        transitions = [['go', 'A', 'C%s1' % (State.separator,)],
                       ['go', 'C', 'D']]
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial='A')
        m.go()
    ...
    def test_ordered_transitions(self):
        ...
        m.next_state()
        m.next_state()
        self.assertEqual('first%sthird' % (State.separator,), m.state)
        m.next_state()
        m.next_state()
        self.assertEqual('first%sfourth%sfifth' % (State.separator,State.separator,), m.state)
        m.next_state()
        m.next_state()
        self.assertEqual('first%sseventh' % (State.separator,), m.state)
        m.next_state()
        m.next_state()
        ...
        m.next_state()
        self.assertEqual(m.state, 'first%ssecond' % (State.separator,))
    ...
    def test_intial_state(self):
        ...
        transitions = [['do', 'A', 'B'],
                       ['do', 'B%s2' % (separator,),
                        'B%s1' % (separator,)]]
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial='A')
        self.assertEqual(m.state, 'A%s2' % (separator,))
        m.do()
        self.assertEqual(m.state, 'B%s2%sa' % (separator,separator,))
    ...
    def test_get_triggers(self):
        ...
        transitions = [
            ...
            ['go', 'standing', 'walking'],
            ['stop', 'walking', 'standing'],
            {'trigger': 'drink', 'source': '*', 'dest': 'caffeinated%sdithering' % (seperator,),
             'conditions': 'is_hot', 'unless': 'is_too_hot'},
            ['walk', 'caffeinated%sdithering' % (seperator,), 'caffeinated%srunning' % (seperator,)],
            ['relax', 'caffeinated', 'standing']
        ]

        machine = self.stuff.machine_cls(states=states, transitions=transitions, auto_transitions=False)
        trans = machine.get_triggers('caffeinated%sdithering' % (seperator,))
...
class TestSeparatorsBase(TestCase):
    ...
    def test_enter_exit_nested(self):
        separator = self.state_cls.separator
        s = self.stuff
        s.machine.add_transition('advance', 'A', 'C%s3' % (separator,))
        s.machine.add_transition('reverse', 'C', 'A')
        s.machine.add_transition('lower', ['C%s1' % (separator,),
                                           'C%s3' % (separator,)], 'C%s3%sa' % (separator,separator,))
        s.machine.add_transition('rise', 'C%s3' % (separator,), 'C%s1' % (separator,))
        s.machine.add_transition('fast', 'A', 'C%s3%sa' % (separator,separator,))
        ...
        s.advance()
        self.assertEqual('C%s3' % (separator,), s.state)
        self.assertEqual(2, s.level)
        self.assertEqual(3, s.transitions)  # exit A; enter C,3
        s.lower()
        self.assertEqual(s.state, 'C%s3%sa' % (separator,separator,))
    ...
    def test_state_change_listeners(self):
        ...
        s.machine.add_transition('reverse', 'C', 'A')
        s.machine.add_transition('lower', 'C%s1' % State.separator, 'C%s3%sa' % (State.separator,State.separator,))
        s.machine.add_transition('rise', 'C%s3' % State.separator, 'C%s1' % State.separator)
        s.machine.add_transition('fast', 'A', 'C%s3%sa' % (State.separator, State.separator,))
        s.machine.on_enter_C('hello_world')
        s.machine.on_exit_C('goodbye')
        s.machine.on_enter('C%s3%sa' % (State.separator,State.separator,), 'greet')
        s.machine.on_exit('C%s3' % State.separator, 'meet')
        s.advance()
        self.assertEqual(s.state, 'C%s1' % State.separator)
        self.assertEqual(s.message, 'Hello World!')
        s.lower()
        self.assertEqual(s.state, 'C%s3%sa' % (State.separator,State.separator,))
    ...
    def test_example_two(self):
        ...
        self.assertEqual(machine.state, 'C%s3%sa' % (separator,separator,))
        machine.to('C%s2' % (separator,))  # exit C↦3↦a, exit C↦3, enter C↦2
        self.assertEqual(machine.state, 'C%s2' % (separator,))
