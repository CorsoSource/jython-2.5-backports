            ...
            if isinstance(state, string_types):
                ...
                if len(domains) > 1:
                    ...
                    if state in self.states:
                        raise ValueError("State %s cannot be added since it already exists." % (state,))
                ...
                if state.name in self.states:
                    raise ValueError("State %s cannot be added since it already exists." % (state.name,))
                self.states[new_state.name] = new_state
                self._init_state(new_state)
                ...
                raise ValueError("Cannot add state of type %s. " % (type(state).__name__,))
    ...
    def get_global_name(self, state=None, join=True):
        ...
        if state:
            ...
            if state_name in self.states:
                ...
                raise ValueError("State '%s' not found in local states." % (state,))
