...
from transitions.compat import NullHandler, property
...
class Machine(object):
    ...
    def __init__(self, model='self', states=None, initial='initial', transitions=None,
        ...
        try:
            ...
            raise ValueError('Passing arguments %s caused an inheritance error: %s' % (kwargs.keys(), err))
    ...
    def _add_model_to_state(self, state, model):
        ...
        for callback in self.state_cls.dynamic_methods:
            method = "%s_%s" % (callback, state.name)
    ...
    def remove_transition(self, trigger, source="*", dest="*"):
        ...
        tmp = dict((key, value) 
                   ...
                   dict((k, [t for t in v
                        ...
                        for k, v 
                        in self.events[trigger].transitions.items()
                       ).items()
                   if len(value) > 0
                  )
    ...
    def __getattr__(self, name):
        ...
        if name.startswith('__'):
            raise AttributeError("'%s' does not exist on <Machine@%s>" % (name, id(self)))
        ...
        if callback_type is not None:
            if callback_type in self.transition_cls.dynamic_methods:
                if target not in self.events:
                    raise AttributeError("event '%s is not registered on <Machine@%s>" % (target, id(self)))
