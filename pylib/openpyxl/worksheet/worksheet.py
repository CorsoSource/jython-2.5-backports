...
class Worksheet(_WorkbookChild):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            if not all([key.start, key.stop]):
                raise IndexError("%s is not a valid coordinate or range" % key)
            key = "%s:%s" % (key.start, key.stop)
    ...
    def unmerge_cells(self, range_string=None, start_row=None, start_column=None, end_row=None, end_column=None):
        ...
        if cr.coord not in self.merged_cells:
            raise ValueError("Cell range %s is not merged" % cr.coord)
    ...
    def _invalid_row(self, iterable):
        raise TypeError('Value must be a list, tuple, range or generator, or a dict. Supplied value is %s' % type(iterable))
