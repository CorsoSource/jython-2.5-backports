...
from openpyxl.descriptors.base import MinMax#, Sequence
from openpyxl.descriptors.sequence import Sequence
from openpyxl.descriptors.serialisable import Serialisable, Strict
...
class CellRange(Serialisable):
    ...
    def __init__(self, range_string=None, min_col=None, min_row=None,
        ...
        if min_col > max_col:
            fmt = "%(max_col)s must be greater than %(min_col)s"
            raise ValueError(fmt % dict(min_col=min_col, max_col=max_col))
        if min_row > max_row:
            fmt = "%(max_row)s must be greater than %(min_row)s"
            raise ValueError(fmt % dict(min_row=min_row, max_row=max_row))
    ...
    def coord(self):
        ...
        if (self.min_col == self.max_col and self.min_row == self.max_row):
            fmt = "%(min_col)s%(min_row)s"
        else:
            fmt = "%(min_col)s%(min_row)s:%(max_col)s%(max_row)s"
    ...
    def __repr__(self):
        ...
        return safe_repr(fmt % dict(cls=self.__class__.__name__, title=self.title, coord=self.coord))
    ...
    def _get_range_string(self):
        ...
        title = self.title
        if title:
            fmt = u"%(title)s!%(coord)s"
            title = quote_sheetname(title)
        return fmt % dict(title=title, coord=self.coord)
    ...
    def shift(self, col_shift=0, row_shift=0):
        ...
        if (self.min_col + col_shift <= 0
            ...
            raise ValueError("Invalid shift value: col_shift=%s, row_shift=%s" % (col_shift, row_shift))
    ...
    def intersection(self, other):
        ...
        if self.isdisjoint(other):
            raise ValueError("Range %s doesn't intersect %s" % (self, other))
