...
FORMAT_REGEX = re.compile("%s|%s|%s" % (FONT_PATTERN, COLOR_PATTERN, SIZE_REGEX) )
...
class _HeaderFooterPart(Strict):
    ...
    * &nn   Selects the specified 2-digit font point size
    ...
    def __str__(self):
        ...
        fmt = []
        if self.font:
            fmt.append(u'&"%s"' % self.font)
        if self.size:
            fmt.append("&%s " % self.size)
        if self.color:
            fmt.append("&K%s" % self.color)
...
class HeaderFooterItem(Strict):
    ...
    def __str__(self):
        ...
        SUBS_REGEX = re.compile("|".join(["(%s)" % (re.escape(k))
                                          for k in TRANSFORM]))
        ...
        for key, part in zip(
            ...
            if part.text is not None:
                txt.append(u"&%s%s" % (key, unicode(part)))
        txt = "".join(txt)
