...
TABLESTYLES = tuple(
    ["TableStyleMedium%s" % i for i in range(1, 29)]
    + ["TableStyleLight%s" % i for i in range(1, 22)]
    + ["TableStyleDark%s" % i for i in range(1, 12)]
)

PIVOTSTYLES = tuple(
    ["PivotStyleMedium%s" % i for i in range(1, 29)]
    + ["PivotStyleLight%s" % i for i in range(1, 29)]
    + ["PivotStyleDark%s" % i for i in range(1, 29)]
)
...
class Table(Serialisable):
    ...
    def _initialise_columns(self):
        ...
        for idx in range(min_col, max_col+1):
            col = TableColumn(id=idx, name="Column%s" % idx)
