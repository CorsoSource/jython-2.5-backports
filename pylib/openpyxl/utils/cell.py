...
SHEETRANGE_RE = re.compile("""%s(?P<cells>%s)(?=,?)""" % (
...
def absolute_coordinate(coord_string):
    ...
    if not m:
        raise ValueError("%s is not a valid coordinate range" % coord_string)
...
def _get_column_letter(col_idx):
    ...
    if not 1 <= col_idx <= 18278:
        raise ValueError("Invalid column index %s" % col_idx)
    letters = []
    while col_idx > 0:
...
def get_column_letter(idx,):
    ...
    try:
        ...
        raise ValueError("Invalid column index %s" % idx)
...
def column_index_from_string(str_col):
    ...
    try:
        ...
        raise ValueError("%s is not a valid column name" % str_col)
...
def range_boundaries(range_string):
    ...
    """
    msg = "%s is not a valid coordinate or range" % range_string
...
def rows_from_range(range_string):
    ...
    for row in rows:
        yield tuple('%s%s' % (col, row) for col in cols)
...
def cols_from_range(range_string):
    ...
    cols = (get_column_letter(col) for col in range(min_col, max_col+1))
    for col in cols:
        yield tuple('%s%s' % (col, row) for row in rows)
