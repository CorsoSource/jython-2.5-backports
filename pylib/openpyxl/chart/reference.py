...
from openpyxl.descriptors.serialisable import Strict
...
class Reference(Strict):
    ...
    def __str__(self):
        if (self.min_col == self.max_col
            ...
            return u"%s!$%s$%s" % (self.sheetname, get_column_letter(self.min_col), self.min_row)
        else:
            return u"%s!$%s$%s:$%s$%s" % (self.sheetname, 
    ...
    def pop(self):
        ...
        """
        cell = "%s%s" % (get_column_letter(self.min_col), self.min_row)
        if self.min_row == self.max_row:
            self.min_col += 1
        else:
            self.min_row += 1
