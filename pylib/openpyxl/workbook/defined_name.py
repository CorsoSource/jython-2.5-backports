...
RESERVED_REGEX = re.compile(r"^_xlnm\.(?P<name>%s)" % _names)
...
TITLES_REGEX = re.compile("""%s%s?,?%s?""" % (SHEET_TITLE, ROW_RANGE, COL_RANGE),
                          re.VERBOSE)
...
class DefinedNameList(Serialisable):
    ...
    def __getitem__(self, name):
        ...
        defn = self.get(name)
        if not defn:
            raise KeyError("No definition called %s" % name)
        return defn
    ...
    def __delitem__(self, name):
        ...
        if not self.delete(name):
            raise KeyError("No globally defined name %s" % name)
