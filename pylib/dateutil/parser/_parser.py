    ...
    def __init__(self, instream):
        ...
        if isinstance(instream, text_type):
            ...
            raise TypeError('Parser must be a string or character stream, not '
                            '%(itype)s' % dict(itype=instream.__class__.__name__))
...
class _ymd(list):
    ...
    def _resolve_from_stridxs(self, strids):
        ...
        out = dict((key,  self[strids[key]]) for key in strids)
    ...
    def resolve_ymd(self, yearfirst, dayfirst):
        ...
        strids = dict((key,  val) for key, val in strids if val is not None)
...
class parser(object):
    ...
    def _build_tzaware(self, naive, res, tzinfos):
        if (callable(tzinfos) or (tzinfos and res.tzname in tzinfos)):
            ...
            # with it
            warnings.warn("tzname %(tzname)s identified but not understood.  "
                          "Pass `tzinfos` argument in order to correctly "
                          "return a timezone-aware datetime.  In a future "
                          "version, this will raise an "
                          "exception." % dict(tzname=res.tzname),
...
class _tzparser(object):
    ...
    def parse(self, tzstr):
        ...
        res.any_unused_tokens = not set(l[n] for n in unused_idxs).issubset({",",":"})
        return res
