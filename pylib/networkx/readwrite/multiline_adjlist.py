...
def parse_multiline_adjlist(lines, comments='#', delimiter=None,
    ...
    for line in lines:
        ...
        try:
            ...
            raise TypeError("Failed to read node and degree on line ({})" % (line,))
        ...
        for i in range(deg):
            while True:
                try:
                    ...
                    msg = "Failed to find neighbor for node ({})" % (u,)
                    raise TypeError(msg)
                p = line.find(comments)
                if p >= 0:
                    line = line[:p]
            ...
            if nodetype is not None:
                try:
                    ...
                    raise TypeError(
                        "Failed to convert node ({}) to type {}" % (v, nodetype,))
            if edgetype is not None:
                try:
                    edgedata = {'weight': edgetype(data)}
                except:
                    raise TypeError(
                        "Failed to convert edge data ({}) to type {}" % (data, edgetype,))
            else:
                try:  # try to evaluate
