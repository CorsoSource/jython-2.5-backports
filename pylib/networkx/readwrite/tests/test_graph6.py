...
class TestWriteGraph6(TestCase):
    ...
    def test_null_graph(self):
        ...
        self.assertEqual(result.getvalue(), '>>graph6<<?\n')
    ...
    def test_complete_bipartite_graph(self):
        ...
        self.assertEqual(result.getvalue(), 'N??F~z{~Fw^_~?~?^_?\n')
    ...
    def test_write_path(self):
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.seek(0)
            self.assertEqual(f.read(), '>>graph6<<?\n')

    def test_relabeling(self):
        G = nx.Graph([(0, 1)])
        assert_equal(g6.to_graph6_bytes(G), '>>graph6<<A_\n')
        G = nx.Graph([(1, 2)])
        assert_equal(g6.to_graph6_bytes(G), '>>graph6<<A_\n')
        G = nx.Graph([(1, 42)])
        assert_equal(g6.to_graph6_bytes(G), '>>graph6<<A_\n')
