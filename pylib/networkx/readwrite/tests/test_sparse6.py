...
class TestWriteSparse6(TestCase):
    ...
    def test_empty_graph(self):
        ...
        result = BytesIO()
        nx.write_sparse6(G, result)
        self.assertEqual(result.getvalue(), '>>sparse6<<:D\n')
    ...
    def test_no_header(self):
        ...
        self.assertEqual(result.getvalue(), ':CcKI\n')

    def test_padding(self):
        codes = (':Cdv', ':DaYn', ':EaYnN', ':FaYnL', ':GaYnLz')
        for n, code in enumerate(codes, start=4):
            G = nx.path_graph(n)
            result = BytesIO()
            nx.write_sparse6(G, result, header=False)
            self.assertEqual(result.getvalue(), code + '\n')
    ...
    def test_write_path(self):
        ...
        self.assertEqual(fh.read(), '>>sparse6<<:?\n')
        fh.close()
        import os
