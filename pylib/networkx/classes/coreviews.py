...
class AtlasView(Mapping):
    ...
    def copy(self):
        return dict((n, self[n].copy()) for n in self._atlas)
...
class UnionAtlas(Mapping):
    ...
    def copy(self):
        result = dict((nbr, dd.copy()) for nbr, dd in self._succ.items())
    ...
    def __str__(self):
        return str(dict((nbr, self[nbr]) for nbr in self))
...
class UnionAdjacency(Mapping):
    ...
    def copy(self):
        return dict((n, self[n].copy()) for n in self._succ)
...
class UnionMultiInner(UnionAtlas):
    ...
    def copy(self):
        ...
        return dict((n, self[n].copy()) for n in nodes)
...
class FilterAtlas(Mapping):  # nodedict, nbrdict, keydict
    ...
    def copy(self):
        try:  # check that NODE_OK has attr 'nodes'
            ...
            node_ok_shorter = False
        if node_ok_shorter:
            return dict((u, self._atlas[u]) for u in self.NODE_OK.nodes
                    if u in self._atlas)
        return dict((u, d) for u, d in self._atlas.items()
                if self.NODE_OK(u))
...
class FilterAdjacency(Mapping):   # edgedict
    ...
    def __getitem__(self, node):
        ...
        raise KeyError("Key {} not found" % (node,))
    ...
    def copy(self):
        try:  # check that NODE_OK has attr 'nodes'
            ...
            node_ok_shorter = False
        if node_ok_shorter:
            return dict((u, dict((v, d) for v, d in self._atlas[u].items()
                        if self.NODE_OK(v) if self.EDGE_OK(u, v)))
                    for u in self.NODE_OK.nodes if u in self._atlas)
        return dict((u, dict((v, d) for v, d in nbrs.items() if self.NODE_OK(v)
                    if self.EDGE_OK(u, v)))
...
class FilterMultiInner(FilterAdjacency):  # muliedge_seconddict
    ...
    def __getitem__(self, nbr):
        ...
        raise KeyError("Key {} not found" % (nbr,))
    ...
    def copy(self):
        try:  # check that NODE_OK has attr 'nodes'
            ...
            node_ok_shorter = False
        if node_ok_shorter:
            return dict((v, dict((k, d) for k, d in self._atlas[v].items()
                        if self.EDGE_OK(v, k)))
                    for v in self.NODE_OK.nodes if v in self._atlas)
        return dict((v, dict((k, d) for k, d in nbrs.items() if self.EDGE_OK(v, k)))
...
class FilterMultiAdjacency(FilterAdjacency):  # multiedgedict
    ...
    def copy(self):
        ...
        if node_ok_shorter:
            ...
            return dict((u, dict((v, dict((k, d) for k, d in kd.items()
