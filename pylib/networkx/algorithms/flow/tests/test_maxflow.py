...
msgi = "Assertion failed in function: %s in interface %s"
...
def validate_flows(G, s, t, flowDict, solnValue, capacity, flow_func):
    assert_equal(set(G), set(flowDict), msg=msg % (flow_func.__name__,))
    for u in G:
    ...
    for u in flowDict:
        for v, flow in flowDict[u].items():
            ...
            ok_(flow >= 0, msg=msg % (flow_func.__name__,))
            excess[u] -= flow
            excess[v] += flow
    for u, exc in excess.items():
        if u == s:
            assert_equal(exc, -solnValue, msg=msg % (flow_func.__name__,))
        elif u == t:
            assert_equal(exc, solnValue, msg=msg % (flow_func.__name__,))
        else:
            assert_equal(exc, 0, msg=msg % (flow_func.__name__,))
...
def compare_flows_and_cuts(G, s, t, solnFlows, solnValue, capacity='capacity'):
    for flow_func in flow_funcs:
        ...
        assert_equal(flow_value, solnValue, msg=msg % (flow_func.__name__,))
        ...
        for interface_func in interface_funcs:
            for flow_func in flow_funcs:
                ...
                if interface_func in max_min_funcs:
                    result = result[0]
                assert_equal(fv, result, msg=msgi % (flow_func.__name__,
        ...
        for flow_func in [shortest_augmenting_path, edmonds_karp]:
            for cutoff in [3, 2, 1]:
                ...
                assert_equal(cutoff, result,
                             msg="cutoff error in %s" % (flow_func.__name__,))
