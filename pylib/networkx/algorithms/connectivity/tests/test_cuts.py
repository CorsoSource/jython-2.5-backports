...
def test_brandes_erlebach_book():
    ...
    for flow_func in flow_funcs:
        ...
        assert_equal(2, len(edge_cut), msg=msg % (flow_func.__name__,))
        H = G.copy()
        ...
        assert_equal(2, len(node_cut), msg=msg % (flow_func.__name__,))
        H = G.copy()
...
def test_white_harary_paper():
    ...
    for flow_func in flow_funcs:
        ...
        # edge cuts
        edge_cut = nx.minimum_edge_cut(G, **kwargs)
        assert_equal(3, len(edge_cut), msg=msg % (flow_func.__name__,))
        H = G.copy()
        H.remove_edges_from(edge_cut)
        assert_false(nx.is_connected(H), msg=msg % (flow_func.__name__,))
        # node cuts
        node_cut = nx.minimum_node_cut(G, **kwargs)
        assert_equal(set([0]), node_cut, msg=msg % (flow_func.__name__,))
        H = G.copy()
        H.remove_nodes_from(node_cut)
        assert_false(nx.is_connected(H), msg=msg % (flow_func.__name__,))
...
def test_petersen_cutset():
    ...
    for flow_func in flow_funcs:
        ...
        # node cuts
        node_cut = nx.minimum_node_cut(G, **kwargs)
        assert_equal(3, len(node_cut), msg=msg % (flow_func.__name__,))
        H = G.copy()
        H.remove_nodes_from(node_cut)
        assert_false(nx.is_connected(H), msg=msg % (flow_func.__name__,))
...
def test_octahedral_cutset():
    ...
    for flow_func in flow_funcs:
        ...
        # edge cuts
        edge_cut = nx.minimum_edge_cut(G, **kwargs)
        assert_equal(4, len(edge_cut), msg=msg % (flow_func.__name__,))
        H = G.copy()
        H.remove_edges_from(edge_cut)
        assert_false(nx.is_connected(H), msg=msg % (flow_func.__name__,))
        # node cuts
        node_cut = nx.minimum_node_cut(G, **kwargs)
        assert_equal(4, len(node_cut), msg=msg % (flow_func.__name__,))
        H = G.copy()
        H.remove_nodes_from(node_cut)
        assert_false(nx.is_connected(H), msg=msg % (flow_func.__name__,))
...
def test_node_cutset_random_graphs():
    for flow_func in flow_funcs:
        for i in range(3):
            ...
            assert_false(nx.is_connected(G), msg=msg % (flow_func.__name__,))
