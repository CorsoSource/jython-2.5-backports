...
msg = "Assertion failed in function: %s"
...
def are_node_disjoint_paths(G, paths):
    ...
    st = set([paths[0][0], paths[0][-1]])
    num_of_nodes = len([n for path in paths for n in path if n not in st])
    num_unique_nodes = len(set([n for path in paths for n in path if n not in st]))
...
def test_graph_from_pr_2053():
    ...
    for flow_func in flow_funcs:
        ...
        assert_true(are_edge_disjoint_paths(G, edge_paths), msg=msg % (flow_func.__name__,))
        assert_equal(
        ...
        assert_true(are_node_disjoint_paths(G, node_paths), msg=msg % (flow_func.__name__,))
        assert_equal(
...
def test_florentine_families():
    ...
    for flow_func in flow_funcs:
        ...
        assert_true(are_edge_disjoint_paths(G, edge_dpaths), msg=msg % (flow_func.__name__,))
        assert_equal(
        ...
        assert_true(are_node_disjoint_paths(G, node_dpaths), msg=msg % (flow_func.__name__,))
        assert_equal(
...
def test_octahedral_disjoint_paths():
    ...
    for flow_func in flow_funcs:
        ...
        assert_true(are_edge_disjoint_paths(G, edge_dpaths), msg=msg % (flow_func.__name__,))
        assert_equal(4, len(edge_dpaths), msg=msg % (flow_func.__name__,))
        # node disjoint paths
        node_dpaths = list(nx.node_disjoint_paths(G, 0, 5, **kwargs))
        assert_true(are_node_disjoint_paths(G, node_dpaths), msg=msg % (flow_func.__name__,))
        assert_equal(4, len(node_dpaths), msg=msg % (flow_func.__name__,))
...
def test_icosahedral_disjoint_paths():
    ...
    for flow_func in flow_funcs:
        ...
        edge_dpaths = list(nx.edge_disjoint_paths(G, 0, 6, **kwargs))
        assert_true(are_edge_disjoint_paths(G, edge_dpaths), msg=msg % (flow_func.__name__,))
        assert_equal(5, len(edge_dpaths), msg=msg % (flow_func.__name__,))
        # node disjoint paths
        node_dpaths = list(nx.node_disjoint_paths(G, 0, 6, **kwargs))
        assert_true(are_node_disjoint_paths(G, node_dpaths), msg=msg % (flow_func.__name__,))
        assert_equal(5, len(node_dpaths), msg=msg % (flow_func.__name__,))
...
def test_cutoff_disjoint_paths():
    ...
    for flow_func in flow_funcs:
        ...
        for cutoff in [2, 4]:
            ...
            edge_dpaths = list(nx.edge_disjoint_paths(G, 0, 6, **kwargs))
            assert_true(are_edge_disjoint_paths(G, edge_dpaths), msg=msg % (flow_func.__name__,))
            assert_equal(cutoff, len(edge_dpaths), msg=msg % (flow_func.__name__,))
            # node disjoint paths
            node_dpaths = list(nx.node_disjoint_paths(G, 0, 6, **kwargs))
            assert_true(are_node_disjoint_paths(G, node_dpaths), msg=msg % (flow_func.__name__,))
            assert_equal(cutoff, len(node_dpaths), msg=msg % (flow_func.__name__,))
