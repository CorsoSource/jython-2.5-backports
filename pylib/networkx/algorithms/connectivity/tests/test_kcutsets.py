...
def test_is_separating_set_complete_graph():
    ...
    assert_true(_is_separating_set(G, set([0, 1, 2, 3])))
...
def test_is_separating_set():
    for i in [5, 10, 15]:
        ...
        assert_true(_is_separating_set(G, set([max_degree_node])))
...
def test_non_repeated_cuts():
    ...
    solution = [set([32, 33]), set([2, 33]), set([0, 3]), set([0, 1]), set([29, 33])]
...
def test_cycle_graph():
    G = nx.cycle_graph(5)
    solution = [set([0, 2]), set([0, 3]), set([1, 3]), set([1, 4]), set([2, 4])]
    cuts = list(nx.all_node_cuts(G))
    assert_true(len(solution) == len(cuts))
    for cut in cuts:
        assert_true(cut in solution)
