    ...
    try:
        ...
        if not infeasible and orig_k < k:
            assert_greater_equal(info['aug_k'], k, (
                'connectivity should increase to k={} or more' % (k,)))
        ...
        print('aug_edges = {}' % (list(aug_edges,)))
        print('info  = {}' % (info,))
        raise
    else:
        if verbose:
...
def _check_augmentations(G, avail=None, max_k=None, weight=None,
    ...
    avail_uniform = dict((e, 1) for e in complement_edges(G))

    if verbose:
        print('\n=== CHECK_AUGMENTATION ===')
        print('G.number_of_nodes = %r' % (G.number_of_nodes(,)))
        print('G.number_of_edges = %r' % (G.number_of_edges(,)))
        print('max_k = %r' % (max_k,))
        print('max_aug_k = %r' % (max_aug_k,))
        print('orig_k = %r' % (orig_k,))
