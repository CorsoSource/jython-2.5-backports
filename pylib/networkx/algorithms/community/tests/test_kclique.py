...
def test_isolated_K5():
    ...
    c = set(k_clique_communities(G, 5))
    assert_equal(c, set([frozenset(range(5)), frozenset(range(5, 10))]))
...
class TestZacharyKarateClub(object):
    ...
    def test_k3(self):
        ...
        comm2 = [0, 4, 5, 6, 10, 16]
        comm3 = [24, 25, 31]
        expected = set([frozenset(comm1), frozenset(comm2), frozenset(comm3)])
        self._check_communities(3, expected)

    def test_k4(self):
        expected = set([frozenset([0, 1, 2, 3, 7, 13]), frozenset([8, 32, 30, 33]),
                    frozenset([32, 33, 29, 23])])
        self._check_communities(4, expected)

    def test_k5(self):
        expected = set([frozenset([0, 1, 2, 3, 7, 13])])
