...
class TestGirvanNewman(object):
    ...
    def test_no_edges(self):
        ...
        validate_communities(communities[0], [set([0]), set([1]), set([2])])
    ...
    def test_undirected(self):
        ...
        validate_communities(communities[0], [set([0, 1]), set([2, 3])])
        # After the next, we get the graph .-. . ., but there are two
        # symmetric possible versions.
        validate_possible_communities(communities[1], [set([0]), set([1]), set([2, 3])],
                                      [set([0, 1]), set([2]), set([3])])
        # After the last removal, we always get the empty graph.
        validate_communities(communities[2], [set([0]), set([1]), set([2]), set([3])])
    ...
    def test_most_valuable_edge(self):
        ...
        assert_equal(len(communities), 3)
        validate_communities(communities[0], [set([0]), set([1, 2, 3])])
        validate_communities(communities[1], [set([0]), set([1]), set([2, 3])])
        validate_communities(communities[2], [set([0]), set([1]), set([2]), set([3])])
