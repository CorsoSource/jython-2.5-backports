...
from networkx.compat import combinations, chain
...
def test_one_node():
    ...
    result = set([frozenset(c) for c in communities])
...
class TestAsynLpaCommunities(object):
    ...
    def test_several_communities(self):
        ...
        ground_truth = set([frozenset(range(3 * i, 3 * (i + 1))) for i in range(5)])
