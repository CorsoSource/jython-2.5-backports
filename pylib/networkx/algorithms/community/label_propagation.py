...
def asyn_lpa_communities(G, weight=None, seed=None):
    ...
    labels = dict((n, i) for i, n in enumerate(G))
    cont = True
    while cont:
        cont = False
        nodes = list(G)
...
def label_propagation_communities(G):
    ...
    labeling = dict((v, k) for k, v in enumerate(G))
...
def _most_frequent_labels(node, labeling, G):
    ...
    if not G[node]:
        ...
        return set([labeling[node]])

    # Compute the frequencies of all neighbours of node
    freqs = Counter(labeling[q] for q in G[node])
    max_freq = max(freqs.values())
    return set([label for label, freq in freqs.items() if freq == max_freq])
