...
from networkx.compat import next, combinations, chain
...
def triangles(G, nodes=None):
    ...
    return dict((v, t // 2) for v, d, t, _ in _triangles_and_degree_iter(G, nodes))
...
def _triangles_and_degree_iter(G, nodes=None):
    ...
    for v, v_nbrs in nodes_nbrs:
        vs = set(v_nbrs) - set([v])
        gen_degree = Counter(len(vs & (set(G[w]) - set([w]))) for w in vs)
    ...
    if G.is_directed():
        if weight is not None:
            td_iter = _directed_weighted_triangles_and_degree_iter(
                G, nodes, weight)
            clusterc = dict((v, 0 if t == 0 else t / ((dt * (dt - 1) - 2 * db) * 2))
        ...
        if weight is not None:
            ...
            clusterc = dict((v, 0 if t == 0 else t / (d * (d - 1))) for
                        v, d, t in td_iter)
        else:
...
3: Counter({3: 4}), 4: Counter({3: 4})}
    ...
    return dict((v, gd) for v, d, t, gd in _triangles_and_degree_iter(G, nodes))
