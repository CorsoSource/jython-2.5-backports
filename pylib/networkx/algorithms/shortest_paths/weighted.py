...
def single_source_dijkstra_path(G, source, cutoff=None, weight='weight'):
    ...
    """
    return multi_source_dijkstra_path(G, set([source]), cutoff=cutoff,
                                      weight=weight)
...
def single_source_dijkstra_path_length(G, source, cutoff=None,
    ...
    """
    return multi_source_dijkstra_path_length(G, set([source]), cutoff=cutoff,
                                             weight=weight)
...
def single_source_dijkstra(G, source, target=None, cutoff=None,
    ...
    single_source_bellman_ford()
    """
    return multi_source_dijkstra(G, set([source]), cutoff=cutoff, target=target,
                                 weight=weight)
...
def multi_source_dijkstra(G, sources, target=None, cutoff=None,
    ...
    if target in sources:
        return (0, [target])
    weight = _weight_function(G, weight)
    paths = dict((source, [source]) for source in sources)  # dictionary of paths
    ...
    try:
        ...
        raise nx.NetworkXNoPath("No path to {}." % (target,))
...
def _dijkstra_multisource(G, sources, weight, pred=None, paths=None,
    ...
    for source in sources:
        if source not in G:
            raise nx.NodeNotFound("Source {} not in G" % (source,))
        seen[source] = 0
...
def _bellman_ford(G, source, weight, pred=None, paths=None, dist=None,
    ...
    for s in source:
        if s not in G:
            raise nx.NodeNotFound("Source {} not in G" % (s,))

    if pred is None:
        pred = dict((v, []) for v in source)

    if dist is None:
        dist = dict((v, 0) for v in source)
...
def bidirectional_dijkstra(G, source, target, weight='weight'):
    ...
    if source not in G or target not in G:
        ...
        raise nx.NodeNotFound(msg % (source, target,))
...
def johnson(G, weight='weight'):
    ...
    dist = dict((v, 0) for v in G)
    pred = dict((v, []) for v in G)
    weight = _weight_function(G, weight)
    ...
    return dict((v, dist_path(v)) for v in G)
