...
class MultiDiGraph_EdgeKey(nx.MultiDiGraph):
    ...
    def add_edge(self, u_for_edge, v_for_edge, key_for_edge, **attr):
        ...
        if key in self.edge_index:
            ...
            if (u != uu) or (v != vv):
                raise Exception("Key %r is already in use." % (key,))
    ...
    def remove_edge_with_key(self, key):
        try:
            ...
            raise KeyError('Invalid edge key %r' % (key,))
        else:
            del self.edge_index[key]
...
class Edmonds(object):
    ...
    def __init__(self, G, seed=None):
        ...
        self.template = random_string(seed=seed) + '_%s'
    ...
    def find_optimum(self, attr='weight', default=1, kind='max',
            ...
            if edge is None:
                ...
                if acceptable:
                    ...
                    if Q_edges is not None:
                        ...
                        # Now we mutate it.
                        new_node = self.template % (self.level,)
        ...
        while self.level > 0:
            ...
            merged_node = self.template % (self.level,)
...
maximum_branching.__doc__ = \
    docstring_branching % dict(kind='maximum', style='branching',)

minimum_branching.__doc__ = \
    docstring_branching % dict(kind='minimum', style='branching',)

maximum_spanning_arborescence.__doc__ = \
    docstring_arborescence % dict(kind='maximum', style='spanning arborescence',)

minimum_spanning_arborescence.__doc__ = \
    docstring_arborescence % dict(kind='minimum', style='spanning arborescence',)
