...
class TestIsEdgeCover:
    ...
    def test_graph_with_loop(self):
        G = nx.Graph()
        G.add_edge(1, 1)
        assert_true(nx.is_edge_cover(G, set([(1, 1)])))

    def test_graph_single_edge(self):
        G = nx.Graph()
        G.add_edge(0, 1)
        assert_true(nx.is_edge_cover(G, set([(0, 0), (1, 1)])))
        assert_true(nx.is_edge_cover(G, set([(0, 1), (1, 0)])))
        assert_true(nx.is_edge_cover(G, set([(0, 1)])))
        assert_false(nx.is_edge_cover(G, set([(0, 0)])))
