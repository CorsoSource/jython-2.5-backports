...
def test_all_simple_paths():
    ...
    assert_equal(set(tuple(p) for p in paths), set([(0, 1, 2, 3)]))
...
def test_all_simple_paths_cutoff():
    ...
    assert_equal(set(tuple(p) for p in paths), set([(0, 1)]))
    paths = nx.all_simple_paths(G, 0, 1, cutoff=2)
    assert_equal(set(tuple(p) for p in paths), set([(0, 1), (0, 2, 1), (0, 3, 1)]))
...
def test_all_simple_paths_multigraph():
    ...
    assert_equal(set(tuple(p) for p in paths), set([(1, 2), (1, 2), (1, 10, 2)]))


def test_all_simple_paths_multigraph_with_cutoff():
    G = nx.MultiGraph([(1, 2), (1, 2), (1, 10), (10, 2)])
    paths = nx.all_simple_paths(G, 1, 2, cutoff=1)
    assert_equal(set(tuple(p) for p in paths), set([(1, 2), (1, 2)]))
...
def test_all_simple_paths_directed():
    ...
    assert_equal(set(tuple(p) for p in paths), set([(1, 2, 3)]))
...
def test_weighted_shortest_simple_path():
    ...
    weight = dict(((u, v), random.randint(1, 100)) for (u, v) in G.edges())
