...
class TestStronglyConnected:
    ...
    def setUp(self):
        ...
        C = set([frozenset([3, 4, 5, 7]), frozenset([1, 2, 8]), frozenset([6])])
        self.gc.append((G, C))

        G = nx.DiGraph()
        G.add_edges_from([(1, 2), (1, 3), (1, 4), (4, 2), (3, 4), (2, 3)])
        C = set([frozenset([2, 3, 4]), frozenset([1])])
        self.gc.append((G, C))

        G = nx.DiGraph()
        G.add_edges_from([(1, 2), (2, 3), (3, 2), (2, 1)])
        C = set([frozenset([1, 2, 3])])
        self.gc.append((G, C))
        ...
        C = set([frozenset([0, 1, 2]), frozenset([3, 4])])
        self.gc.append((G, C))

    def test_tarjan(self):
        scc = nx.strongly_connected_components
        for G, C in self.gc:
            assert_equal(set([frozenset(g) for g in scc(G)]), C)
