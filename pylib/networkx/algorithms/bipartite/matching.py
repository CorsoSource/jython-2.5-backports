...
def hopcroft_karp_matching(G, top_nodes=None):
    ...
    leftmatches = dict((v, None) for v in left)
    rightmatches = dict((v, None) for v in right)
    distances = {}
    ...
    leftmatches = dict((k, v) for k, v in leftmatches.items() if v is not None)
    rightmatches = dict((k, v) for k, v in rightmatches.items() if v is not None)
...
def _connected_by_alternating_paths(G, matching, targets):
    ...
    edge_sets = set([frozenset((u, v)) for u, v in matching.items()])
    matched_edges = set([tuple(edge) for edge in edge_sets])
    unmatched_edges = set([(u, v) for (u, v) in G.edges()
                       if frozenset((u, v)) not in edge_sets])

    return set([v for v in G if v in targets or
