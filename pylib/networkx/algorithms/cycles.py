...
def _min_cycle(G, orth, weight=None):
    ...
    nodes_idx = dict((node, idx) for idx, node in enumerate(G.nodes()))
    idx_nodes = dict((idx, node) for node, idx in nodes_idx.items())
    ...
    cross_paths_w_lens = dict((n, all_shortest_pathlens[n][nnodes + n])
    ...
    return set([frozenset((idx_nodes[u], idx_nodes[v])) for u, v in mcycle_pruned])
