...
from networkx.algorithms.coloring.greedy_coloring_with_interchange import greedy_coloring_with_interchange as _interchange
...
def strategy_saturation_largest_first(G, colors):
    ...
    """
    distinct_colors = dict((v, set()) for v in G)
    for i in range(len(G)):
        ...
        if i == 0:
            ...
            saturation = dict((v, len(c)) for v, c in distinct_colors.items()
                          if v not in colors)
...
def greedy_color(G, strategy='largest_first', interchange=False):
    ...
    for u in nodes:
        ...
        neighbour_colors = set([colors[v] for v in G[u] if v in colors])
