...
class Node(object):
    ...
    def __repr__(self):
        return "Node_id: %s, Color: %s, Adj_list: (%s), \
            adj_color: (%s)" % (
...
class AdjEntry(object):
    ...
    def __repr__(self):
        return "Node_id: %s, Next: (%s), Mate: (%s), \
            col_next: (%s), col_prev: (%s)" % (
            self.node_id,
            self.next,
            self.mate.node_id,
...
def greedy_coloring_with_interchange(original_graph, nodes):
    ...
    graph = dict((node_id, Node(node_id, n)) for node_id in original_graph)
    ...
    for node in nodes:
        ...
        col_used = set([graph[adj_node.node_id].color for adj_node in neighbors])
        col_used.discard(-1)
