...
def main_inner(popts, args, usage):
    ...
    if args:
        ...
        if not lexer:
            print >>sys.stderr, 'Error: when using -s a lexer has to be selected with -l'
            return 2
...
def main(args=sys.argv):
    ...
    try:
        ...
        if '-v' in dict(popts):
            print >>sys.stderr
            print >>sys.stderr, '*' * 65
            print >>sys.stderr, 'An unhandled exception occurred while highlighting.'
            print >>sys.stderr, 'Please report the whole traceback to the issue tracker at'
            print >>sys.stderr, '<https://github.com/pygments/pygments/issues>.'
            print >>sys.stderr, '*' * 65
            print >>sys.stderr
            raise
        import traceback
        ...
        print >>sys.stderr, '*** If this is a bug you want to report, please rerun with -v.'
        return 1
