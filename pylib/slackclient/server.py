...
class Server(object):
    ...
    def api_call(self, token, request="?", timeout=None, **kwargs):
        ...
        response_json = {}
        resp_text = response if isinstance(response, (str,unicode)) else response.text
        if resp_text:
            response_json = system.util.jsonDecode(resp_text)
        response_json["headers"] = dict(response_json if isinstance(response, (str,unicode)) else response.headers )
        return system.util.jsonEncode(response_json)
