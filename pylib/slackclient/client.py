...
class SlackClient(object):
    ...
    def refresh_access_token(self):
        ...
        response_json = system.util.jsonDecode(response.text)
    ...
    def api_call(self, method, timeout=None, **kwargs):
        ...
        try:
            result = system.util.jsonDecode(response_body)
        except ValueError, json_decode_error:
            raise ParseResponseError(response_body, json_decode_error)
        response_json = system.util.jsonDecode(response_body)
        ...
        if self.server:
            ...
            if json_data != "":
                for d in json_data.split("\n"):
                    data.append(system.util.jsonDecode(d))
            for item in data:
